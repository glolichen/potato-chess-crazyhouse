...
ull get_attacked(const bitboard::Position &board, bool color) {
...
ull get_pinned(const bitboard::Position &board, bool color) {
	...
	if (blockers) {
		...
		if (blockers && QUERY(rook_queen, 63 - __builtin_clzll(~maps::rook[__builtin_ctzll(blockers)][0] & maps::rook[king_pos][0]))) {
	...
	if (blockers) {
		...
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(rook_queen, __builtin_ctzll(~maps::rook[63 - __builtin_clzll(blockers)][1] & maps::rook[king_pos][1]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
	...
	if (blockers) {
		pinned_piece = 63 - __builtin_clzll(blockers);
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(rook_queen, __builtin_ctzll(~maps::rook[63 - __builtin_clzll(blockers)][2] & maps::rook[king_pos][2]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
	...
	if (blockers) {
		pinned_piece = __builtin_ctzll(blockers);
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(rook_queen, 63 - __builtin_clzll(~maps::rook[__builtin_ctzll(blockers)][3] & maps::rook[king_pos][3]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
	...
	if (blockers) {
		pinned_piece = __builtin_ctzll(blockers);
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(bishop_queen, 63 - __builtin_clzll(~maps::bishop[__builtin_ctzll(blockers)][0] & maps::bishop[king_pos][0]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
	...
	if (blockers) {
		pinned_piece = 63 - __builtin_clzll(blockers);
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(bishop_queen, __builtin_ctzll(~maps::bishop[63 - __builtin_clzll(blockers)][1] & maps::bishop[king_pos][1]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
	...
	if (blockers) {
		pinned_piece = 63 - __builtin_clzll(blockers);
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(bishop_queen, __builtin_ctzll(~maps::bishop[63 - __builtin_clzll(blockers)][2] & maps::bishop[king_pos][2]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
	...
	if (blockers) {
		pinned_piece = __builtin_ctzll(blockers);
		SET0(blockers, pinned_piece);
		if (blockers && QUERY(bishop_queen, 63 - __builtin_clzll(~maps::bishop[__builtin_ctzll(blockers)][3] & maps::bishop[king_pos][3]))) {
			ull potential = 1ull << pinned_piece;
			if (potential & own_pieces)
				pinned |= potential;
		}
...
void get_fill_moves(const bitboard::Position &board, std::vector<int> &moves, ull blocks) {
	for (int i = 0; i < 6; i++) {
		if (!board.reserve[board.turn][i])
			continue;

		int opponent_king = __builtin_ctzll(board.pieces[!board.turn][KING]);
		ull no_go_squares;
		switch (i) {
			case 0:
				no_go_squares = moveGen::get_pawn_attacks(opponent_king, !board.turn);
				break;
			case 1:
				no_go_squares = moveGen::get_knight_attacks(opponent_king);
				break;
			case 2:
				no_go_squares = moveGen::get_bishop_attacks(board, board.all_pieces, opponent_king);
				break;
			case 3:
				no_go_squares = moveGen::get_rook_attacks(board, board.all_pieces, opponent_king);
				break;
			case 4:
				no_go_squares = moveGen::get_bishop_attacks(board, board.all_pieces, opponent_king) | moveGen::get_rook_attacks(board, board.all_pieces, opponent_king);
				break;
			default:
				no_go_squares = 0;
		}

		no_go_squares |= (~blocks);

		for (int j = 0; j < 64; j++) {
			if (i == PAWN && (j/8 == 0 || j/8 == 7))
				continue;
			if (QUERY(board.all_pieces, j) || QUERY(no_go_squares, j))
				continue;
			moves.push_back(NEW_FILL_MOVE(j, (i + board.turn * 6)));
		}
	}
}
...
void moveGen::move_gen(const bitboard::Position &board, std::vector<int> &moves) {
	...
	ull attacked = get_attacked(board, board.turn);
	ull checks = get_checks(board, board.turn);
	ull pinned = get_pinned(board, board.turn);
	ull pieces, blocks;
	...
	get_fill_moves(board, moves, blocks);
}
...
std::vector<move::Move> moveGen::move_gen_for_JS(std::string fen) {
	...
	for (const int &move : moves_int) {
		if (IS_FILL(move))
			moves.push_back({ -1, 63 - DEST(move), 0, 0, 0, PIECE(move) + 1, 1 });
		else
			moves.push_back({ 63 - SOURCE(move), 63 - DEST(move), CASTLE(move), PROMOTE(move), (bool) IS_EP(move), 0, 0 });
	}
